...
export default function View(){
    ...
    const {deleteBbs,bbsInfo,dispatch}=useContext(BbsContext);
    const {loading} = bbsInfo;
    ...
    useEffect(()=>{
        ...
        const fetchBbs = async () => {
            try{
                ...
                dispatch({type:BBS.LOADING,loading:false});
            }
            catch(err){
                console.log(err);
                setError(err);
        ...
        <div className="text-center">
            ...
            */}
            ...
            <button className="btn btn-success mx-2" onClick={()=>{
                ...
                if(window.confirm('정마로 삭제 하시겠습니까?')) deleteBbs(bbs.id);
            }}>삭제</button>
